...
function rock:draw()
	if self.timers.birth < 31 then
		...
		love.graphics.setColor(170,120,85,self.timers.birth*8)
	else
		love.graphics.setColor(170,120,85)
	end
	-- love.graphics.draw(self.p)
	love.graphics.draw(self.image,self.x,self.y,0,1,1,50,50)
	if self.sentry then
...
--
	...
	if self.hp < self._hp and self.hp >= self._hp/2 then
		-- if self.p:count() == 0 then
		-- 	self.p:setBufferSize(4*(1-(self.hp/self._hp)))
		-- 	self.p:setParticleLife(6)
...
--	print()
	for i,p in ipairs(state.players) do
		if p.distance > math.max(p.members.a.attraction,p.members.b.attraction) then
		for ii,m in pairs(p.members) do
--			print(math.floor(((m.x-self.x)^2+(m.y-self.y)^2)^.5), m.attraction)
			if math.floor((((m.x-self.x)^2)+((m.y-self.y)^2))^.5) <= m.attraction then
--				self.x = self.x + math.floor(10/(m.x-self.x))
--				self.y = self.y + math.floor(10/(m.y-self.y))
				self.x = self.x + math.round(math.abs(math.cos(math.atan2((m.y-self.y),(m.x-self.x)))*m.attraction)/((m.x-self.x)),1)
				self.y = self.y + math.round(math.abs(math.sin(math.atan2((m.y-self.y),(m.x-self.x)))*m.attraction)/((m.y-self.y)),1)--((m.y-self.y)/100)
			end
		end
		end
	end
